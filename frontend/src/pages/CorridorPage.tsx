...
export function CorridorPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 md:px-8">
      <h1 className="text-2xl font-semibold text-white">{t("corridor.title")}</h1>
      <p className="mt-2 text-sm text-slate-400">{t("corridor.subtitle")}</p>
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-end">
        <div className="flex-1">
          <label className="block text-xs font-medium text-slate-400">{t("corridor.chainLabel")}</label>
          <input
            value={raw}
            ...
            placeholder={t("corridor.placeholder")}
          />
        </div>
        <button
          type="button"
        ...
        >
          {busy ? t("corridor.analyzing") : t("corridor.analyze")}
        </button>
      </div>
      ...
      {result?.valid === true ? (
        <div className="mt-6 space-y-4 rounded-2xl border border-surface-600/60 bg-surface-800/40 p-4">
          <p className="font-mono text-lg text-white">
            {t("corridor.scoreLabel")} {String(result.corridor_risk_score)} ({String(result.corridor_risk_level)})
          </p>
          <p className="text-sm text-slate-300">
            {t("corridor.bottleneck")}{" "}
            <strong>{String((result.bottleneck as { name_en?: string })?.name_en)}</strong> (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm text-slate-300">
              <thead className="text-xs uppercase text-slate-500">
                <tr>
                  <th className="py-2">{t("corridor.tableCountry")}</th>
                  <th className="py-2">{t("corridor.tableScore")}</th>
                  <th className="py-2">{t("corridor.tableLevel")}</th>
                  <th className="py-2">{t("corridor.tableLogistics")}</th>
                </tr>
              </thead>
              <tbody>
          ...
          Object.keys(result.recommendation_playbook as object).length > 0 ? (
            <div className="mt-6 border-t border-surface-600/50 pt-4">
              <h3 className="text-sm font-semibold text-slate-200">{t("corridor.playbookTitle")}</h3>
