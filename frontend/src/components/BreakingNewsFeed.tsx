  ...
  return (
    <div className="flex h-full min-h-[320px] flex-col rounded-2xl border border-surface-600/60 bg-surface-800/30">
      <div className="flex items-center gap-2 border-b border-surface-600/60 px-4 py-3">
        ...
        <div>
          <h2 className="text-sm font-semibold text-slate-200">Son dakika &amp; dünya gündemi</h2>
          <p className="text-xs text-slate-500">BBC World + Google News TR — ~15 dk yenilenir</p>
        </div>
      </div>
      <div className="flex-1 space-y-2 overflow-y-auto p-4 pr-2" style={{ maxHeight: 560 }}>
        {err ? <p className="text-sm text-rose-300">{err}</p> : null}
        {!err && items.length === 0 ? (
          <p className="text-sm text-slate-500">Haberler yükleniyor…</p>
        ) : null}
        {items.map((it, i) => (
          <article
            key={`${it.link}-${i}`}
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-[11px] text-slate-500">
                {it.published_at ? new Date(it.published_at).toLocaleString("tr-TR") : "—"}
              </span>
            </div>
